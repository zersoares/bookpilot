...
test("every result the callbacks use yields a working page", async () => {
  for (const provider of ["meta", "pinterest", "tiktok", "google", "amazon"]) {
...
test("the callbacks no longer use HTTP redirects", () => {
  for (const file of ["bookpilot-meta.mjs", "bookpilot-pinterest.mjs", "bookpilot-tiktok.mjs", "bookpilot-google.mjs", "bookpilot-amazon.mjs"]) {
    ...
    assert.match(source, /backToApp\("(meta|pinterest|tiktok|google|amazon)", "connected"\)/, `${file} reports success through backToApp`);
  }
});
