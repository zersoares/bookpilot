// Automatic Pinterest sync, on the Pinterest card.
//
// Read-only, and the screen says so before it asks for anything: BookPilot
// requests one Pinterest permission (ads:read), can see campaign figures,
// and cannot create, change, pause or spend on anything. The CSV import
// below it stays as the way in when this is not set up, or as a fallback.
//
// Honest about its own state: when the deployment has no Pinterest app
// credentials, this says the connection isn't set up rather than offering a
// button that fails.
    ...
    return html`
      <div data-pc>
        ...
        <p class="bp-small bp-muted" style="margin:0 0 var(--bp-3)">
          Connect your Pinterest ad account and BookPilot pulls campaign spend, clicks and checkouts by
          day, so you don't have to download reports. It asks for <strong>one read-only permission</strong>
          (<code>ads:read</code>): it can see your campaign figures and can't create, change, pause or
          spend on anything. Pinterest bills you directly, and BookPilot never sees your password.
        </p>
        ${raw(integration?.status === "expired" ? '<div class="bp-alert bp-alert--warning" style="margin-bottom:var(--bp-3)"><span class="bp-alert__icon">!</span><div class="bp-small">Your Pinterest connection expired. Connect again to keep syncing.</div></div>' : "")}
        <button type="button" class="bp-btn bp-btn--primary bp-btn--sm" data-pc-action="connect">Connect Pinterest</button>
      </div>`;
  }
  ...
  return html`
    <div data-pc>
      ...
      <dl class="bp-kv">
        ...
        <dt>Access expires</dt><dd>${integration.expires_at ? fmt.date(integration.expires_at) : "—"}</dd>
      </dl>
      ...
      ${raw(integration.account_id ? html`
        <div class="bp-grid bp-grid--2" style="gap:var(--bp-3);margin-top:var(--bp-3)">
          <label class="bp-field" style="margin:0"><span class="bp-label">Days to pull</span>
            ...
            <span class="bp-tiny bp-subtle">Pinterest allows up to 90.</span></label>
          <label class="bp-field" style="margin:0"><span class="bp-label">Book for new campaigns</span>
            ...
            <span class="bp-tiny bp-subtle">Only used for Pinterest campaigns BookPilot hasn't seen before.</span></label>
        </div>` : "")}
      ...
      <p class="bp-tiny bp-subtle" style="margin:var(--bp-3) 0 0">
        Figures use Pinterest's default conversion windows (30 days after a click) and count
        <strong>checkouts</strong> as purchases. Importing a report for the same days replaces them.
      </p>
    </div>`;
}

export function wirePinterestConnect(root, { books = [], done }) {
  const block = root.querySelector('[data-platform-card][data-platform="pinterest"] [data-pc]');
  if (!block) return;
  ...
  block.addEventListener("click", async (event) => {
    ...
    if (action === "connect") {
      ...
      try {
        const { url } = await API.pinterestAuthorizeUrl();
        location.href = url;
      } catch (err) { notify.error(err.message); button.disabled = false; }
    } else if (action === "accounts") {
      button.disabled = true;
      try {
        const { accounts, selected } = await API.pinterestAccounts();
        ...
        list.innerHTML = accounts.length
          ? accounts.map((a) => html`
              <label class="bp-radio ${a.id === selected ? "bp-radio--selected" : ""}">
                <input type="radio" name="pin-account" value="${a.id}" ${a.id === selected ? "checked" : ""}>
                <span><strong>${a.name}</strong>
                  <span class="bp-small bp-subtle"> · ${a.currency || "currency not stated"}${a.country ? ` · ${a.country}` : ""}</span></span>
              </label>`).join("") +
          ...
          : '<p class="bp-small bp-muted">No ad accounts found on that Pinterest login.</p>';
      } catch (err) { notify.error(err.message); }
      button.disabled = false;
    } else if (action === "use-account") {
      const chosen = block.querySelector('input[name="pin-account"]:checked');
      if (!chosen) return;
      try {
        await API.pinterestSelectAccount({ account_id: chosen.value });
      ...
      result.innerHTML = '<span class="bp-small bp-muted">Syncing from Pinterest…</span>';
      try {
        const r = await API.pinterestSync({ days: Number(field("days")?.value) || 30, book_id: field("book")?.value || undefined });
      ...
      const ok = await confirmDialog({
        ...
        message: "BookPilot forgets the connection and stops syncing. Figures already imported stay. Pinterest keeps a record that BookPilot was allowed access, so to remove it there too, do so in your Pinterest account's connected-apps settings.",
        confirmLabel: "Disconnect", tone: "danger",
      });
      if (!ok) return;
      try { await API.pinterestDisconnect(); notify.success("Pinterest disconnected."); done?.(); }
      catch (err) { notify.error(err.message); }
    }
  });
}
