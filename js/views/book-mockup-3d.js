...
const resolved = new Map(); // coverUrl -> data URL, once rendered
 ...
 * same in-flight render rather than starting a second one.
 */
export async function renderMockup3D(coverUrl) {
  ...
  const promise = (async () => {
    ...
    // camera a little low, the way a held or displayed book actually turns.
    const cam = makeCamera({ W, H, yaw: 30, pitch: 15, cx: W * 0.5, cy: H * 0.86, zoom: 1 });
    closedBook(ctx, cam, asset, { w: bw, h: bh, d, lay: "upright", yaw: 0, at: [0, 0], pal: { shadow: "#1a1410", dark: false }, S: 1 });

    // A soft diagonal key-light sweep over the book's own pixels only —
    // the difference between a flat render and one that reads as lit.
